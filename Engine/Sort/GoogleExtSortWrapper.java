...
*
* Usage: 
*  java com/google/code/externalsorting/Sorter somefile.txt out.txt
* 
* You can change the default maximal number of temporary files with the -t flag:
*  java com/google/code/externalsorting/Sorter somefile.txt out.txt -t 3
*
* For very large files, you might want to use an appropriate flag to allocate
* more memory to the Java VM: 
*  java -Xms2G com/google/code/externalsorting/Sorter somefile.txt out.txt
*
...
public class GoogleExtSortWrapper extends ExtSortWrapper{
	private static int DEFAULTMAXTEMPFILES = 1024;
	private Comparator<String> comparator;
	
	
	
	public GoogleExtSortWrapper(String inFileName, String outFileName, LinkedList<Integer> fields, String orderby, LinkedList<String> fieldsType){
		super(inFileName, outFileName);

		if(orderby.equalsIgnoreCase("asc"))
			 comparator = new GoogleAscComparator(fields, fieldsType);
		else
			comparator = new GoogleDescComparator(fields, fieldsType);
	...
	public void sort() {
		...
		List<File> l;
		try {
			l = sortInBatch(inFile, comparator, maxtmpfiles,cs,null);
			mergeSortedFiles(l, outFile, comparator,cs);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
